...
async function generate(cells, current) {
    // SETUP FOR THE STACK AND INITIALIZE THE FIRST CELL AS VISITED
    let stack = [];
    ...
    while (stack.length > 0) {
        ...
        // IF IT HAS UNVISITED NEIGHBOURS, PUSH THEM TO THE STACK
        if (cellNeighbours.length > 0) {
            stack.push(cell);

            // RANDOMLY PICK ONE OF THE NEIGHBOURS
            let randomIndex = Math.floor(Math.random() * cellNeighbours.length);
            let nextCell = cellNeighbours[randomIndex];

            // GO TO THE RANDOMLY CHOSEN NEIGHBOUR AND PUSH IT TO THE STACK
